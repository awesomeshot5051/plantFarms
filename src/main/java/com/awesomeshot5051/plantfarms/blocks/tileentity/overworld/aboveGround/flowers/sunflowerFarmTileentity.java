...
public class sunflowerFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public sunflowerFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.SUNFLOWER_FARM.get(), ModBlocks.SUNFLOWER_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static int getSunflowerSpawnTime() {
        return Main.SERVER_CONFIG.sunflowerSpawnTime.get() - 20 * 4;
    }

    public static int getSunflowerDeathTime() {
        return getSunflowerSpawnTime() + 20 * 4; // 30 seconds spawn time + 10 seconds kill time
    }
    ...
    public void tick() {
        ...
        if (timer == getSunflowerSpawnTime()) {
            sync();
//            }
        } else if (timer >= getSunflowerDeathTime()) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.SUNFLOWER, dropCount)); // Drop 1 blaze rod
