...
public class MangroveFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public MangroveFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.MANGROVE_FARM.get(), ModBlocks.MANGROVE_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static double getMangroveSpawnTime(MangroveFarmTileentity farm) {
        AxeType axe = AxeType.fromItem(farm.getAxeType().getItem());
        return (double) Main.SERVER_CONFIG.mangroveSpawnTime.get() / (axe.equals(AxeType.NETHERITE) ? 30 : axe.equals(AxeType.DIAMOND) ? 25 : axe.equals(AxeType.GOLDEN) ? 20 : axe.equals(AxeType.IRON) ? 15 : axe.equals(AxeType.STONE) ? 10 : 1);
    }

    public static double getMangroveDeathTime(MangroveFarmTileentity farm) {
        AxeType axe = AxeType.fromItem(farm.getAxeType().getItem());
        ...
        return getMangroveSpawnTime(farm) + (axe.equals(AxeType.NETHERITE) ? (baseValue * 3.2) : axe.equals(AxeType.DIAMOND) ? (baseValue * 5.6) : axe.equals(AxeType.IRON) ? (baseValue * 4.8) : axe.equals(AxeType.STONE) ? (baseValue * 6.4) : axe.equals(AxeType.WOODEN) ? (baseValue * 6.4) : 6.4);
    }
    ...
    private List<ItemStack> getDrops() {
        ...
        if (serverWorld.random.nextDouble() < dropChance) {
            ...
            drops.add(new ItemStack(Items.MANGROVE_ROOTS, dropCount));
        }
        drops.add(new ItemStack(Items.MANGROVE_LOG));
        if (serverWorld.random.nextDouble() < .005) {
            drops.add(new ItemStack(Items.STRIPPED_MANGROVE_LOG));
        }
        drops.add(new ItemStack(Items.MANGROVE_LEAVES));
        return drops;
    }
