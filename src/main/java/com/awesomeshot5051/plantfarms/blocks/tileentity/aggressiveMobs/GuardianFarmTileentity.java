...
public class GuardianFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public GuardianFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.GUARDIAN_FARM.get(), ModBlocks.GUARDIAN_FARM.get().defaultBlockState(), pos, state);
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.GUARDIAN_PRIMED);
...
//            BlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.GUARDIAN_PRIMED);
            sync();
//        } else if (timer > getGuardianSpawnTime() && timer < getGuardianExplodeTime()) {
//            if (timer % 20L == 0L) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.GUARDIAN_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        LootParams.Builder builder = new LootParams.Builder(serverWorld)
                .withParameter(LootContextParams.THIS_ENTITY, new Guardian(EntityType.GUARDIAN, level))
        ...
        LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(GUARDIAN_LOOT_TABLE);
