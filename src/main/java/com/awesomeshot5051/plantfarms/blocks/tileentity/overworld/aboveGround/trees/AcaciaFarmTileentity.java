...
public class AcaciaFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getAcaciaDeathTime(AcaciaFarmTileentity farm) {
        ...
        return getAcaciaSpawnTime(farm) + (axe.equals(AxeType.NETHERITE) ? (baseValue * 3.2) : axe.equals(AxeType.DIAMOND) ? (baseValue * 5.6) : axe.equals(AxeType.IRON) ? (baseValue * 4.8) : axe.equals(AxeType.STONE) ? (baseValue * 6.4) : axe.equals(AxeType.WOODEN) ? (baseValue * 6.4) : 6.4);
    }
    ...
    private List<ItemStack> getDrops() {
        ...
        if (serverWorld.random.nextDouble() < dropChance) {
            ...
            drops.add(new ItemStack(Items.ACACIA_SAPLING, dropCount));
        }
