...
//private static final ResourceKey<LootTable> ACACIA_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/acacia"));
    ...
    public orchidFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.ORCHID_FARM.get(), ModBlocks.ORCHID_FARM.get().defaultBlockState(), pos, state);
    ...
    public static int getOrchidSpawnTime() {
        return Main.SERVER_CONFIG.orchidSpawnTime.get() - 20 * 4;
    }

    public static int getOrchidDeathTime() {
        return getOrchidSpawnTime() + 20 * 4; // 30 seconds spawn time + 10 seconds kill time
    }
    ...
    public void tick() {
        ...
        if (timer == getOrchidSpawnTime()) {
            sync();
//            }
        } else if (timer >= getOrchidDeathTime()) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.BLUE_ORCHID, dropCount)); // Drop 1 blaze rod
