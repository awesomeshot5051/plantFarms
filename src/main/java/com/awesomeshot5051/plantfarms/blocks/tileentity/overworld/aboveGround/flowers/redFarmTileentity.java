...
public class redFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
...
//private static final ResourceKey<LootTable> ACACIA_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/acacia"));
    ...
    public redFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.RED_FARM.get(), ModBlocks.RED_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static int getRedSpawnTime() {
        return Main.SERVER_CONFIG.redSpawnTime.get() - 20 * 4;
    }

    public static int getRedDeathTime() {
        return getRedSpawnTime() + 20 * 4; // 30 seconds spawn time + 10 seconds kill time
    }
    ...
    public void tick() {
        ...
        if (timer == getRedSpawnTime()) {
            sync();
//            }
        } else if (timer >= getRedDeathTime()) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.RED_TULIP, dropCount)); // Drop 1 blaze rod
