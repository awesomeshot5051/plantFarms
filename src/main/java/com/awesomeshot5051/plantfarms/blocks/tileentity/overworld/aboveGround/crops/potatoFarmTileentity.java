...
public class potatoFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public potatoFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.POTATO_FARM.get(), ModBlocks.POTATO_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static double getPotatoSpawnTime(potatoFarmTileentity farm) {
        HoeType hoe = HoeType.fromItem(farm.getHoeType().getItem());
        return (double) Main.SERVER_CONFIG.potatoSpawnTime.get() / (hoe.equals(HoeType.NETHERITE) ? 30 : hoe.equals(HoeType.DIAMOND) ? 25 : hoe.equals(HoeType.GOLDEN) ? 20 : hoe.equals(HoeType.IRON) ? 15 : hoe.equals(HoeType.STONE) ? 10 : 1);
    }

    public static double getPotatoDeathTime(potatoFarmTileentity farm) {
        HoeType hoe = HoeType.fromItem(farm.getHoeType().getItem());
        ...
        return getPotatoSpawnTime(farm) + (hoe.equals(HoeType.NETHERITE) ? (baseValue * 3.2) : hoe.equals(HoeType.DIAMOND) ? (baseValue * 5.6) : hoe.equals(HoeType.IRON) ? (baseValue * 4.8) : hoe.equals(HoeType.STONE) ? (baseValue * 6.4) : hoe.equals(HoeType.WOODEN) ? (baseValue * 6.4) : 6.4);
    }
    ...
    public void tick() {
        timer++;
        setChanged();
        if (timer == getPotatoSpawnTime(this)) {
            sync();
        } else if (timer >= getPotatoDeathTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        if (serverWorld.random.nextDouble() < dropChance) {
            ...
            drops.add(new ItemStack(Items.POISONOUS_POTATO, dropCount));
        }
