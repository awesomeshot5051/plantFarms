...
public class berryFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {

//private static final ResourceKey<LootTable> BERRY_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/berry"));
    ...
    public berryFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.BERRY_FARM.get(), ModBlocks.BERRY_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static double getBerrySpawnTime(berryFarmTileentity farm) {
        HoeType hoe = HoeType.fromItem(farm.getHoeType().getItem());
        return (double) Main.SERVER_CONFIG.berrySpawnTime.get() /
    ...
    public static double getBerryDeathTime(berryFarmTileentity farm) {
        ...
        return getBerrySpawnTime(farm) + (hoe.equals(HoeType.NETHERITE) ? (baseValue * 3.2) :
...
//        BlockBase.playRandomVillagerSound(level, getBlockPos(), SoundEvents.BERRY_PRIMED);

        timer++;
        setChanged();

        if (timer == getBerrySpawnTime(this)) {
//            // Play berry spawn sound
//            BlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.BERRY_PRIMED);
            sync();
//        } else if (timer > getBerrySpawnTime() && timer < getBerryDeathTime()) {
//            if (timer % 20L == 0L) {
//                BlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.BERRY_HURT);
//            }
        } else if (timer >= getBerryDeathTime(this)) {
            // Play berry death/explosion sound
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.BERRY_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.SWEET_BERRIES, dropCount)); // Drop 1 blaze rod
