...
public class poppyFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
...
//private static final ResourceKey<LootTable> ACACIA_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/acacia"));
    ...
    public poppyFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.POPPY_FARM.get(), ModBlocks.POPPY_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static int getPoppySpawnTime() {
        return Main.SERVER_CONFIG.poppySpawnTime.get() - 20 * 4;
    }

    public static int getPoppyDeathTime() {
        return getPoppySpawnTime() + 20 * 4; // 30 seconds spawn time + 10 seconds kill time
    }
...
//            }
        } else if (timer >= getPoppyDeathTime()) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.ACACIA_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.POPPY, dropCount)); // Drop 1 blaze rod
