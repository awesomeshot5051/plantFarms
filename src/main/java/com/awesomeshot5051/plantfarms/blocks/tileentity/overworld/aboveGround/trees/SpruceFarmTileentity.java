...
public class SpruceFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getSpruceSpawnTime(SpruceFarmTileentity farm) {
        AxeType axe = AxeType.fromItem(farm.getAxeType().getItem());
        return (double) Main.SERVER_CONFIG.spruceSpawnTime.get() / (axe.equals(AxeType.NETHERITE) ? 30 : axe.equals(AxeType.DIAMOND) ? 25 : axe.equals(AxeType.GOLDEN) ? 20 : axe.equals(AxeType.IRON) ? 15 : axe.equals(AxeType.STONE) ? 10 : 1);
    }

    public static double getSpruceDeathTime(SpruceFarmTileentity farm) {
        AxeType axe = AxeType.fromItem(farm.getAxeType().getItem());
        ...
        return getSpruceSpawnTime(farm) + (axe.equals(AxeType.NETHERITE) ? (baseValue * 3.2) : axe.equals(AxeType.DIAMOND) ? (baseValue * 5.6) : axe.equals(AxeType.IRON) ? (baseValue * 4.8) : axe.equals(AxeType.STONE) ? (baseValue * 6.4) : axe.equals(AxeType.WOODEN) ? (baseValue * 6.4) : 6.4);
    }
    ...
    private List<ItemStack> getDrops() {
        ...
        if (serverWorld.random.nextDouble() < dropChance) {
            ...
            drops.add(new ItemStack(Items.SPRUCE_SAPLING, dropCount));
        }
