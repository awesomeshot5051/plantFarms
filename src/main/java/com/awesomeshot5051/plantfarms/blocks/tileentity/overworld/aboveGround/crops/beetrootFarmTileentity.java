...
public class beetrootFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public static double getBeetrootDeathTime(beetrootFarmTileentity farm) {
        ...
        return getBeetrootSpawnTime(farm) + (hoe.equals(HoeType.NETHERITE) ? (baseValue * 3.2) : hoe.equals(HoeType.DIAMOND) ? (baseValue * 5.6) : hoe.equals(HoeType.IRON) ? (baseValue * 4.8) : hoe.equals(HoeType.STONE) ? (baseValue * 6.4) : hoe.equals(HoeType.WOODEN) ? (baseValue * 6.4) : 6.4);
    }

    public static double getBeetrootSpawnTime(beetrootFarmTileentity farm) {
        HoeType hoe = HoeType.fromItem(farm.getHoeType().getItem());
        return (double) Main.SERVER_CONFIG.beetrootSpawnTime.get() / (hoe.equals(HoeType.NETHERITE) ? 30 : hoe.equals(HoeType.DIAMOND) ? 25 : hoe.equals(HoeType.GOLDEN) ? 20 : hoe.equals(HoeType.IRON) ? 15 : hoe.equals(HoeType.STONE) ? 10 : 1);
    }
    ...
    private List<ItemStack> getDrops() {
        ...
        if (serverWorld.random.nextDouble() < dropChance) {
            ...
            drops.add(new ItemStack(Items.BEETROOT_SEEDS, dropCount));
        }
