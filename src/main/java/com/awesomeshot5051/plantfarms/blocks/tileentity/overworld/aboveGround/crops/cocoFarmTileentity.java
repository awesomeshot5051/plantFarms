...
public class cocoFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
...
//private static final ResourceKey<LootTable> ACACIA_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/acacia"));
    ...
    public cocoFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.COCO_FARM.get(), ModBlocks.COCO_FARM.get().defaultBlockState(), pos, state);
        inventory = NonNullList.withSize(4, ItemStack.EMPTY);
        itemHandler = new ItemStackHandler(inventory);
        outputItemHandler = new OutputItemHandler(inventory);
    }

    public static int getCocoSpawnTime() {
        return Main.SERVER_CONFIG.cocoaBeanSpawnTime.get() - 20 * 4;
    }

    public static int getCocoDeathTime() {
        return getCocoSpawnTime() + 20 * 4; // 30 seconds spawn time + 10 seconds kill time
    }
...
//            }
        } else if (timer >= getCocoDeathTime()) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.ACACIA_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.COCOA_BEANS, dropCount)); // Drop 1 blaze rod
